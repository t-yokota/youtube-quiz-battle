...
YouTube Quiz Battleは、YouTubeクイズ動画とリアルタイム早押し解答を組み合わせたWebアプリケーションです。プレイヤーは動画の進行に合わせて早押しボタンを押し、制限時間内に解答することで、インタラクティブなクイズ体験を楽しめます。

### Core Concept
```
YouTubeクイズ動画 + リアルタイム早押し解答 = インタラクティブ体験
```

**基本体験**: プレイヤーはYouTubeに投稿されたクイズ動画を視聴しながら、動画内で出題された問題に早押しボタンを押して解答し、動画出演者と疑似的にクイズの対戦を楽しむことができる。

**プレイ可能人数**: 1人（クイズ動画を視聴中に、動画の再生端末を操作することが可能な1名）
...
- **シーク判定方法**: `|currentVideoTime - watchedVideoTime| > currentVideoTimeの更新間隔(100ms)` を満たすとき
...
#### 基本方針
- クライアント側で処理: クライアント側で正解データを所持し、プレイヤーの入力内容との比較によって正誤判定を行う（同期処理）。サーバ通信は行わない。
- 正規化処理: 正誤判定の前処理として、正解データとプレイヤーの入力内容の両方に正規化を実施する。

#### 正規化処理の設定
```typescript
interface AnswerValidationConfig {
  normalizeCase: boolean;          // 大文字小文字を統一するか
  normalizeWidth: boolean;         // 全角半角を統一するか
  trimWhitespace: boolean;         // 前後空白を除去するか
  ignoreSymbols: boolean;          // 記号を無視するか
  
  // 表記揺れ対応（拡張機能）
  enableKanaUnification?: boolean;  // カタカナをひらがなに統一するか（Phase1）
  enableKanjiReading?: boolean;     // 漢字を読み(仮名)に統一するか（Phase2）
}
```
...
| 文字種 | 判定条件 | 適用する処理 |
|--------|---------|---------|
| JAPANESE | ひらがな・カタカナ・漢字含む | 全正規化処理 |
| ALPHABET | 英字メイン | 大文字小文字・全角半角・空白除去 |
| NUMERIC | 数字メイン | 全角半角・空白除去 |
| MIXED | 混合 | 全正規化処理 |

#### 段階的実装
- **Phase 1**: 基本正規化（全角半角統一、カナ統一、空白処理、大文字小文字）
- **Phase 2**: 拡張正規化（漢字読み変換、kuroshiro+kuromojiライブラリ使用）
...
*Guide Text（LOADING/READY/TALKING状態用）*
- LOADING状態：「読み込み中...」
...
*Answer Content（QUESTIONING/ANSWERING/WAITING/REVEALING状態用）*
...
- **Detection Method**: `|currentVideoTime - watchedVideoTime| > currentVideoTimeの更新間隔`でシーク判定
- **Purpose**: 順次視聴の担保

**jumpToRevealPeriod（正解発表区間への遷移設定）**
- **Type**: boolean
- **When true**:
  - 正解時: 正解音再生と同時に`revealTime`へ時間をジャンプして動画再開
  - 不正解かつ解答回数終了時: 不正解音再生と同時に`revealTime`へ時間をジャンプして動画再開
- **When false**: 通常の動画再生を継続
- **Audio Playback**: 動画の時間をジャンプする際も効果音の再生は継続する

**hideVideoPlayerDuringAnswer（解答中の動画表示制御設定）**
- **Type**: boolean  
- **When true**: ANSWERING状態でYouTube動画プレイヤーを非表示
...
### Unit Testing
- **Game State Logic**: 状態遷移ロジックの単体テスト
- **Answer Validation**: 正規化処理と正誤判定のテスト
- **Time Management**: シーク検出と時間管理のテスト
...
#### Phase 1: コア機能
- **基本UI構造**: レスポンシブレイアウトの実装
- **YouTube Player統合**: 動画再生とイベント処理
- **状態管理システム**: ゲーム状態とボタン状態の管理
- **時間管理システム**: 動画時間追跡とシーク検出
...
- **解答システム**: 入力、送信、正誤判定
- **音声システム**: 効果音の再生制御
- **UI状態制御**: ゲーム状態に応じた表示切り替え

#### Phase 3: 体験向上
- **表記揺れ対応**: 解答の正規化処理
- **早押しボタン画像スプライト**: CSSから画像スプライトへの移行
- **エラー処理**: 詳細なエラー分類と復旧
- **パフォーマンス最適化**: モバイル環境での動作改善
