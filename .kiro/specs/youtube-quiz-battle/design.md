...
- start/reveal/end閾値は副作用のある処理を実行
...
**othersAnsweringPeriodsには一回性フラグは不要:**
- 理由: 副作用のある処理を伴わず、状態の切り替えのみを行う
- 消費済み問題（consumed.start=true）では、問題自体に参加できないため（WAITING状態）、othersAnsweringPeriodsの処理は実質的に意味を持たない

**消費済み区間の扱い:**
- 消費済み閾値では、副作用のある処理（onStart/onReveal/onEnd）は実行されない
- 状態遷移のみ行われる（不参加として適切な状態へ遷移）
...
| previousVideoTime | 直前の再生位置（シーク検出用） | currentVideoTime更新後 | シーク検出 |

### Seek Detection via previousVideoTime

**前提**: 本ゲームでは公正な進行のために、シークバーの使用を原則で禁止とする。

previousVideoTimeは、直前の再生位置を記録する変数である。currentVideoTimeの値を更新したあとで、"更新後のcurrentVideoTimeの値"と"更新前のpreviousVideoTimeの値"の比較を行うことで、プレイヤーによるシークバーの使用（による、動画再生時間のジャンプ）を検出する。
...
シーク操作により問題をスキップした場合（consumed.start=trueで start閾値を通過）：
...
### PreviousVideoTime Update Logic
...
timeline
  ...
  section プレイヤーアクション
    シーク実施 : 再生位置が15.0に移動
  section システム処理2
    getCurrentTime()=15.0 : currentVideoTime=15.0<br>previousVideoTime=5.1: シーク検出=true<br>|15.0 - 5.1| > 0.1: seekTo(5.1)<br>previousVideoTime=5.1<br>（維持）
...
timeline
  ...
  section プレイヤーアクション
    シーク実施 : 再生位置を15.0にシーク
  section システム処理2
    getCurrentTime()=15.0<br>(Question1区間内) : currentVideoTime=15.0<br>previousVideoTime=5.1: シーク検出=true<br>|15.0 - 5.1| > 0.1<br>→Question1を消費<br>(start/reveal/end): 状態=WAITING<br>previousVideoTime=15.0
