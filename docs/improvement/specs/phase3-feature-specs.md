...
| A3 | 音声読み込み失敗の扱い | **design.md どおりブロッキングエラーとする（ユーザー裁定 2026-07-03。非ブロッキング案は否決）**。スプライト読込失敗時は `AUDIO_LOAD_FAILED` を throw し、ErrorDialog で再読み込み誘導（Task 20 の errorHandler に接続）。ただし音声が無効設定（soundEnabled=false）の場合は読込自体を遅延/スキップしてよい |
...
| V3 | 小書きかな標準化 | **実施しない**（「っ」と「つ」の同一視は誤陽性リスクが高い。design.md 手順 4 の「小書きかなの標準化」は長音・NFKC 由来の互換文字処理で実用上足りる）。→ design.md から該当行を削除する（このタスクのコミットに含める）。**ユーザー承認済み（2026-07-03）** |
...
export class AudioManager {
  /** スプライトの読み込み。失敗時は Error('AUDIO_LOAD_FAILED') を throw（裁定 A3: 呼び出し側で ErrorDialog へ） */
...
- [ ] 素材読込失敗時に AUDIO_LOAD_FAILED の ErrorDialog（再読み込み誘導）が表示される（裁定 A3）。※Task 19 実装時点で errorHandler（Task 20）未実装の間は throw の伝播確認まででよい
- [ ] 既存テスト無修正パス（audioManager 省略可のため）

## Task 19-2: 音声システムのテスト

`src/services/__tests__/audioManager.test.ts` + `src/stores/__tests__/settingsStore.test.ts`。AudioContext / HTMLAudio / localStorage をモック。検証: 再生・停止・重複再生時の停止→再生・音量変換（level/4）・ON/OFF・フォールバック分岐・読込失敗時に AUDIO_LOAD_FAILED が throw されること・LocalStorage 復元と壊れた JSON の回復。注: 実スプライト位置（0.5s 境界等）の検証はモックでは無意味なので行わない（19-0 完了後の手動確認事項）。
...
- 音声読込失敗（AUDIO_LOAD_FAILED）もここに接続し、ErrorDialog で再読み込み誘導（裁定 A3・ユーザー裁定）
...
19-0 → 19 → 19-2 →（任意時点で 19-3）→ 20 → 20-2 → 20-3 → 20-4 → 21 → 21-2。A3（再読み込み誘導とする）・V3（小書きかな標準化なし）はユーザー裁定済み（2026-07-03）で確認不要。
