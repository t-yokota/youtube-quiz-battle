...
## 実行体制（モデルルーティング・2026-06-13 追加）

オーケストレーター（Fable 5）が設計・計画・監査・レビュー・コミットを担い、実装タスクはサブエージェント（Opus 4.8 / Sonnet 4.6）へ切り出す。切り出し判断は「**仕様の密度 vs 実装の表面積**」: 指示が短く編集量が多いタスクほど切り出し向き。判断中心で編集量が小さいタスクは、サブエージェントのコールドスタートコスト（コンテキスト再把握 + レビュー往復）が本体を上回るため、オーケストレーターがインラインで実行する。

| 担当 | 対象タスク | 理由 |
|---|---|---|
| Fable インライン | C-1 / Wave 1（D-1〜D-10）/ R-0 / R-6 の分割設計 / R-9 / 仕様バグの裁定 / 全 diff レビューとコミット | 判断中心・編集量小。切り出すと逆に高コスト |
| Sonnet サブエージェント | R-1〜R-4 / R-7a / テスト作成（19-2, 20-2, 21-2。vitest-test-writer は sonnet 指定済み）/ D-1 トークン移植 | 指示書に手順まで記載があり機械的。テスト150件が安全網 |
| Opus サブエージェント | R-5 / R-6 実装 / R-7b / 19・20・21 の機能実装 / D-2 デザイン適用 | 実装裁量が中程度。設計はオーケストレーターが固めて渡す |
| Fable 引き取り | 同種の失敗が2回続いたタスク / R-6 の「既存テスト無修正パス」を満たせない場合 | 追加指示で粘るより引き取るほうが安い |
...
2. **コミット権はオーケストレーター**: サブエージェントの作業はワーキングツリーの変更まで。オーケストレーターが diff をレビューし、`Task XX:` 規約でコミットする（1 タスク 1 コミットと品質ゲートの維持）
3. **2ストライクで引き取り**: 同じ失敗を2回報告したサブエージェントには追加指示で粘らず、オーケストレーターが引き取る
...
未決の判断ポイント: D-2 実装時のタイマー表示（①リング / ②解答エリア枠メーター）の選択のみ。
