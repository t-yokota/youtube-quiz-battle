...
**次のタスク: Phase 4（実行順: 27-1 → 22 → 23 → 27-2/3。spec: [improvement/specs/phase4-quality-release.md](specs/phase4-quality-release.md)。25/26 は後回し）**

> Phase 1〜2（Task 1〜18-3）は完了済み → [tasks-phase1-2.md](tasks-phase1-2.md)
> 本リストは [docs/improvement/04-task-replan.md](04-task-replan.md)（2026-06-12 決定反映済み）に基づく。着手時は本ファイルを正とする。
> 実行ルール・実行体制（モデルルーティング）は [docs/improvement/00-overview.md](00-overview.md) を参照。
...
## Phase R: リファクタリング（詳細: [improvement/02-refactoring-plan.md](02-refactoring-plan.md)）
...
## Phase D: デザイン改善（詳細: [improvement/05-design-review.md](05-design-review.md)）
...
実行 spec: [improvement/specs/phase4-quality-release.md](specs/phase4-quality-release.md)（2026-07-03 作成）。実行順は 27-1（base 対応）→ 22 → 23 → 27-2/3 → 実機確認。ホスティングは暫定 GitHub Pages。25/26 は後回し（着手時に spec 作成）。

- [x] 22. モバイル最適化（旧: 22）※実機チェックリスト（spec 22-3）も 2026-07-06 全消化
  - ソフトキーボード対応（visualViewport で ANSWERING 時の入力欄可視性確保）/ タッチ応答 / iOS・Android 実機確認
- [x] 23. PC 対応の仕上げ（旧: 23）※Enter 送信を追加。ナビ一巡は手動確認
  - キーボードナビゲーション一巡 / hover・focus 状態の網羅（D-2 で大半は完了している想定の確認タスク）
- [x] 29. デバッグモード（クイズ設定の実行時上書き）※2026-07-05 追加・完了
  - 実行 spec: [improvement/specs/task29-debug-mode.md](specs/task29-debug-mode.md)
  - settings.debug: true のデータのみ設定モーダルにデバッグセクション（ヘッダー左トグルで表示切替）
  - debugStore（セッション限り）+ gameStore.effectiveSettings で上書きを一元解決
- [x] 25. Firebase Analytics 統合（旧: 25）※実装完了（2026-07-05）。Firebase Console でのプロジェクト作成 + src/constants/firebase.ts への config 貼り付け、DebugView 確認、GA4 UI 登録（spec 25-6）が残ユーザー作業※実行 spec: [improvement/specs/task25-firebase-analytics.md](specs/task25-firebase-analytics.md)（2026-07-05 作成）
  - プレイセッション ID（UUID v4）/ quiz_session_completed / question_answered イベント
  - PrivacyInfo 文言と送信内容の整合確認（D-15）
- [x] 30. quizId パラメータ対応（?quiz= でコンテンツ指定）※2026-07-05 追加・完了
  - 実行 spec: [improvement/specs/task30-quiz-id-param.md](specs/task30-quiz-id-param.md)
  - quizId（slug）と videoId を分離。データは public/data/<quizId>/data.json。Analytics 全イベントに quiz_id
- [~] 26. E2E テストスイート（旧: 26 + 23-2 統合）※**不採用（2026-07-06 ユーザー裁定）**
  - アプリ規模に対して維持コストが見合わないため Playwright 導入を見送り。
    一度実装して全シナリオのパスまで確認した上で取り下げた（spec は
    [improvement/specs/task26-e2e-suite.md](specs/task26-e2e-suite.md) に保存済み。将来必要になれば復活可能）
  - 品質担保は unit テスト（330 件）+ 実機チェックリスト（22-3/27-4）で行う
- [x] 27. パフォーマンス最適化とデプロイ準備（旧: 27）※GitHub Pages + Actions。実機チェックリスト（spec 22-3/27-4）も 2026-07-06 全消化
