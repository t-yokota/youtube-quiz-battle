...
- プレイ画面内のボタンは実行後にフォーカスを解除し、次のSpaceを早押しに使えるようにする。Tabで選択しただけでは解除しない。解答入力欄や、操作によって入力欄へ移ったフォーカスは維持する。設定・テーマ画面内はこの解除の対象外とする。
...
function applyThresholds(prev: number, curr: number, q: QuizQuestion) {
  ...
  if (prev + TIME_EPSILON_SEC < q.endTime && curr + TIME_EPSILON_SEC >= q.endTime) {
    if (!c.end) {
      ...
      // すべての問題が消費済みかチェック
      const allConsumed = questions.every(
        (q) => consumed[q.index]?.start && consumed[q.index]?.reveal && consumed[q.index]?.end,
      )
...
- 上位警告を閉じると下位画面の直前の要素へ戻す。通常は起点の要素へ戻し、起点がなくなった場合は開始ゲートまたは設定ボタンを使う。設定を直接閉じた場合は、背景遮断の解除後に早押しボタンへ戻し、解答中は入力欄、押せない状態ではゲーム操作領域へ移す。これによりSpaceで設定画面が再度開くことを防ぐ。設定→テーマ選択→終了は通常の起点復帰を使う。
- 閉じるアニメーション中の画面は操作対象から外す。unmountでリスナー・監視・背景属性を解放する。
- テーマ一覧と選択時の拡大プレビューは「早押し可能な出題中」の代表表示とする。現在の2D／3D設定、丸型／箱型、BUTTON CHECKの実効ON／OFFを反映する。表示切替アイコンと3D時のタイプ切替アイコンも表示し、プレビュー内では操作を受け付けない。3Dは実画面と共通のモデル・照明・カメラ調整で初期姿勢の透過静止画像を生成し、同じモデル・寸法・画素比・PC判定のカードと拡大表示で共有する。描画後はWebGLを解放し、画像キャッシュは最大12件とする。生成失敗時は2D表示にフォールバックする。解答中のタイマー・動画非表示・プレイ中の回転角度は追従しない。
...
- **Video Player**: 実効設定`hideVideoPlayerDuringAnswer=true`の場合、iframeを保持して隠し、同サイズの領域を「解答中・・・」表示に即時置換する。テーマの背景色に薄く文字色を混ぜ、中央の3点を順番に上下動させる。動きを減らす設定では静止表示にする。タッチ端末でのキーボード向け領域折り畳みは維持する。
- **Game Info Area**: 表示
- **Answer Area**: 解答コンテンツ表示
...
- タッチデバイスでは動画領域を残したままGameInfo・解答エリア・QuizButtonを280msで上へ移動し、完了時に動画領域を畳む。UIは動画の手前に背景付きで重ねる。完了時の高さ除去と移動用transform解除は同じフレームに反映する。途中で解答終了した場合は現在位置から戻し、古い移動完了通知では畳まない。iframeの寸法とインスタンスは維持し、畳んでいる間は`inert`と`aria-hidden`で操作・読み上げの対象から外す。動きを減らす設定では即時切替とする。
- 縦積みレイアウトで動画非表示中は、解答エリアとボタンの間隔を0.875rem、ボタン領域の高さを動画非表示へ移る前の実測値で固定し、キーボード開閉による残り高さに追従させない。早押し経由ではiOSの同期フォーカスより前に測定し、直接遷移ではDOM更新前に測定する。通常表示へ戻ったら固定を解除する。実測できない場合のみ13.5remを使う。動画の高さを補う余白は設けない
...
- サイドのQUESTION／SCOREカードは、余白の最大値1.125remからの縮小量をテーマの角丸から差し引く。角丸はテーマ値の50〜100%に収める。共有の角丸トークンや中央の解答欄、結果行、テーマプレビューには適用しない。
- PCの箱型モデルは、初期角度での投影幅を、高さから求める基準幅の25%以下に制限する。基準幅はmax(480px, ヘッダー下の高さ×13/25×16/9)とし、画面幅の60%制限やサイド幅を差し引く制限は適用しない。自動配置・境界ドラッグとも、横幅を狭めても上限は変えず「拡大→一定」を保つ。ただし実際の領域に収まらなくなる場合は縮小する。高さ変更時は再計算し、再読み込みでも同じ高さなら同じ基準を使う。固定pxの高さ上限は設けず、縦横比と表示中心は維持する。
- PCの丸型モデルは、初期角度での投影幅を、同じ高さ基準幅の30%以下に制限する。上限の再計算条件は箱型と同じ。canvas自体はgame-ui全体を使い、上限未満では領域への追従を維持する。上限適用時も表示中心を動かさない。
- PCの3D canvasはgame-ui全体に重ねて配置する。従来のモデル領域は中心位置の基準として残し、その中心からgame-uiの端まで収まる範囲でサイズを再計算するため、描画領域が広がった分だけ拡大を許可する。解答欄展開時は中心の移動に連動して利用可能範囲を縮める。解答欄・姿勢リセット・タイプ切替・表示切替・BUTTON CHECKはcanvasより前面に置く。
- PCの3D表示はモデルごとの初期角度を基準にカメラを合わせ、回転操作ではカメラを変更しない。丸型・箱型ともカメラの表示位置を8px下寄せし、移動後の構図で安全範囲を計算する。回転軸を中心に全頂点と押下範囲を包む球が8pxの余白内に収まるよう、必要なカメラ距離をあらかじめ確保して回転時のはみ出しを防ぐ。解答エリアの開閉や境界ドラッグでcanvas寸法が変わった場合は、その領域と初期角度で再計算する。スマホは現在の回転角度で領域内に最大表示する処理を維持する。
- 3D箱型の中心位置を決める領域は縮小解答カード用の上余白を設けず、下段上端から1.25remの余白を残して表示する。丸型は従来の領域幅60%への追従を維持し、中心位置を決める領域の幅・高さをそれぞれ最大360pxに制限して中央配置する。2Dとスマホの配置は変更しない。
- 設定画面は右端・画面全高のサイドメニューとして300msでスライド開閉する。ヘッダーを固定し、内容はパネル内でスクロールする。背景クリック・Esc・閉じるボタン、フォーカス制御は既存のモーダル処理を共有する。動きを減らす設定ではアニメーションを省略する。スマホは中央モーダルのまま。
- 中央下段の文字は画面高に連動させず、ブラウザの標準文字サイズ（`medium`、通常16px）と`em`を基準にする。通常設定では入力・送信・ガイド・正誤表示・残り回数・秒数16px、キーヒント・BUTTON CHECKラベル12px。2Dボタン内の文字（PUSH／ON!／WAIT／2行のBUTTON CHECK）は例外としてボタン直径の10%で拡縮する。サイドバーとスマホの文字設計は別系統のまま維持する。文字を大きく設定した場合は解答情報の折り返しと入力・ボタンの高さ拡張を許可する。配色はスマホと同じテーマトークンを共有し、PCだけの文字色・背景色補正は設けない。設計上の参考は[WCAG 1.4.4（200%の文字拡大）](https://www.w3.org/WAI/WCAG22/Understanding/resize-text.html)と[WCAG 1.4.3（通常文字4.5:1）](https://www.w3.org/WAI/WCAG22/Understanding/contrast-minimum.html)。16pxは一律のWCAG必須値ではなく、このUIの基本サイズとして採用する。
- 幅1200px以上かつ主ポインターが`fine`の場合に適用する。iframeでは内部の幅を基準とする。全体は画面幅いっぱいを使い、左右の帯を常に同幅にし、初期動画幅はヘッダーとsafe-areaを除いたmain-contentの実測高さから、動画:UI＝13:12になる動画高を求め、16:9で横幅へ換算する（高さ×13/25×16/9）。920pxの固定上限は設けず、最小640px・左右各240px以上を確保する幅に制限する。制限に当たる場合やUI最低高が必要な低い画面では比率より操作領域の確保を優先する。境界操作後は手動幅を優先する。動画から下段操作領域まで続く左右境界をドラッグすると中心を維持して両サイドが連動する。手動幅は640px以上・左右各240px以上を確保する範囲に制限し、画面幅変更時も収める。境界には下段領域の上下中央に細長いグリップを常時表示し、ホバー・ドラッグ中はアクセント色、フォーカス時は共通フォーカス色で強調する。左右キーでも操作でき、Homeは最小幅・Endは最大幅。手動幅は表示セッション内のみ保持する。スマホ・狭い画面は従来の縦積み表示を維持する。
- 中央に16:9の動画と早押しボタン、右帯にQUESTION・SCORE・RESULTSを表示する。SCOREはResultChipの○／×と各30pxの件数表示とし、確定済みの問題単位で集計する。複数回の誤答は1問として扱い、スキップと解答履歴のないスルーは数えない。現在の問題は「出題中／解答中」の行で示し、正答は結果確定後だけ表示する。結果一覧は内部スクロール可能。RESULTS見出しには追加の上余白0.5remを設ける。
- PCもスマホと同じgamePanelModeに従い、LOADING・READY・TALKINGでは残り回数の代わりにガイド文を表示する。PCではREADYの下向き矢印を表示しない。ガイド文も実測した数値幅を使い、文言変更・解答内容との切り替え・正誤表示の増減でカード幅を連続的に補間する。PCの残り回数は拡大・縮小とも「解答残り N回 / M」に統一する。回数と正誤バッジは共通のまとまりにし、縮小中も同じ配置方式でカードに追従させる。縮小後は中央揃え・要素間1rem、拡大後は従来の左右配置を維持する。縮小後の目標幅は拡大中から計測する。カードの位置・幅・高さとボタン領域は共通の進行率で560msかけて同時に補間し、入力行も遅延なしで同時に展開する。3Dモデル幅の更新では位置の遷移を再始動しない。内部文言だけの幅変更は520msで補間する。正解・不正解の表示条件を満たす間は、縮小時も残り回数の右横に結果を表示する。残り回数と結果表示の文字サイズは縮小・展開で共通とする。正誤バッジは行高・内側余白・角丸も共通にし、拡大時の外観を縮小後も維持する。縮小カードの横幅は結果表示を含む文字の実測幅と内側余白に合わせ、上下0.5rem・左右0.875remの余白と共通の小角丸（--radius-sm）を使う。ボタンの表示領域は縮小カードの下端から始まる。ANSWERINGでカードを右中央へ展開し、入力行を表示する。正誤判定によりANSWERINGを出た後は1秒間維持してから縮小する（ANSWER_PANEL_RESULT_HOLD_MS）。WAITING・REVEALINGなどへの遷移では再拡大も維持時間の延長もしない。再解答時は以前の縮小予約を解除し、次問・終了時は縮小する。正解・不正解の消去タイミングは従来のゲーム進行に従い、カードの開閉では変更しない。展開時は2D／3Dのボタンを左側へ移し、左端からボタン・ボタンから解答カード・カードから右端の余白を等しくする。余白は2Dの台座幅／3Dの投影輪郭幅とカード幅を下段幅から引き、3等分して求める。縮小時はカードを右上へ戻し入力行を隠す。
- 主ポインターが`fine`で幅880〜1199pxの場合、左サイドのみ非表示にした中間レイアウトを使う。中央の左端を画面左に固定し、右サイドを240px以上確保する。動画最小幅は640px、右境界のドラッグ量をそのまま動画幅に反映し、左境界は操作・Tab対象から外す。設定サイドメニューとPCテーマプレビューも維持する。幅880px未満または主ポインターが`fine`でない場合はスマホの縦積み配置に切り替える。
- 動画の自動初期幅には、上記の高さ基準に加えて画面幅の60%上限を適用する。通常の最小幅は640pxとし、手動ドラッグでは60%上限を適用しない。テーマプレビューの動画幅にも同じ計算を使う。3Dモデルの上限は動画の実幅とは分け、前述の高さ基準幅から求める。
- PCで縦幅が足りない場合は、下段の操作領域16remと動画下辺1pxを先に確保し、残った高さ×16/9で動画幅を制限する。この場合は640px下限を緩め、480pxまで縮小する。手動ドラッグの上限・Home/End・aria値も同じ制限を使う。
- pointer:fineの画面で通常配置が縦に収まらない場合は、中央を幅:高さ=1:2のスマホ形式に切り替える。幅880px以上では「480px動画の高さ270px＋下辺1px＋操作領域16rem」を必要高とする。880px未満でも対象とし、ウインドウ幅の16:9動画＋進行表示・解答欄・余白12.75rem＋ボタン最低高max(118px,7rem)で判定する。代替表示中の中央幅ではなく外側の幅を使うため、切替後に解消して元に戻る往復は発生しない。左右は操作不可の余白とし、サイドバーと境界ドラッグを表示しない。通常配置が収まれば元の表示と手動指定幅を復元する。解答中の動画非表示もスマホの折り畳み動作を使う。終了時はスマホの結果画面を使うが、fineポインターでは判定する高さを一定に保つためヘッダーは維持する。設定画面の内部スクロールは維持し、極端な低さや文字拡大で最低限の操作領域が収まらない場合もスクロールを禁止しない。タッチ端末の従来の表示条件は変更しない。
- スマホの早押しボタン領域の最低高は2Dでmax(88px, 7rem)、3Dでmax(118px, 7rem)とし、余裕がなければ領域に合わせてボタン表示を縮める。アイコンの操作領域、入力文字の下限、動画非表示中のボタン高さ固定は維持する。PCの3D回転中に倍率を変えない仕様も維持する。
- PCの解答カードとの余白には、拡大開始時に毎回記録した回転角度の投影輪郭幅を使う。拡大後の回転には追従せず、開閉やリサイズ中の寸法変化には記録した角度で再計算する。モデル切替時は切替先の姿勢を記録する。カメラの倍率・サイズ上限は初期角度基準を維持する。押下演出による配置の揺れを防ぐため、幅の測定には押し込み前の形状を使う。
- 早押し後の演出待ち時間と解答タイマーは既存ロジックを使用する。PCの入力フォーカスはカード移動完了の560ms後とし、解答終了・コンポーネント破棄で予約を解除する。モーダル表示中はフォーカスしない。動きを減らす設定では移動とフォーカス待ちを省略する。iOSを含むスマホのフォーカス処理は即時のまま。
- 下段に2D／3D切替・キーヒント・BUTTON CHECKを配置し、切替の操作領域は幅44px・高さ36pxとする。動画非表示設定ではPCの動画領域の高さを維持する。PCのFINISHEDでは動画と右側の結果一覧を残し、縮小カード自体を「もう一度プレイ」ボタンに置き換える。早押しボタンは無効状態で表示を残し、表示切替・キーヒント・BUTTON CHECKも維持する。文字サイズ・余白・角丸は縮小カードと共通とし、再プレイでREADYに戻す。スマホでは既存の結果画面を維持する。
- テーマ選択の一覧・拡大プレビューも同じPC判定を用いる。中央動画と下段、右側のQUESTION／SCORE／RESULTS、縮小解答カードとキーヒントを代表表示する。カードはviewportと同じ縦横比の2列表示で最大幅560px。3DはPCの初期姿勢・回転余白・サイズ上限を用いて静止画像を生成し、スマホ用とはキャッシュを分ける。スマホでは従来の縦積みプレビューを維持する。
- 基準は1440×900。低いPC画面では中央の最低操作領域を確保してスクロールを許可し、操作要素の重なりを防ぐ。Light・Dark・Neumorphismの既存テーマトークンを使用する。

### フォーカスと設定画面の外観

- フォーカス色は共通の`--color-focus`で指定し、デフォルトは青`#356fca`、ダークは青`#4f8cff`、ニューモーフィズムは紫`#6c63ff`とする。解答入力欄の境界線・影は形を維持して同色に揃える。それ以外の枠は2pxで、トグルはラベルを囲まずトラックの輪郭だけに表示する。設定ボタンの枠は内側に収め、表示切替は四角い枠にする。動画幅境界もフォーカス時は共通色を使い、ホバー・ドラッグ時はアクセント色を使う。
- 設定画面のプルダウン・通常ボタンは`0.75 × settings-layout-unit`の角丸で統一する。ニューモーフィズムの外周シャドウは控えめにし、結果リストには影が切れない描画余白を確保する。

### 3D早押しボタン

- 姿勢リセットのアイコンはスマホ24px・PC32px、操作領域はタイプ切替・2D／3D切替と共通の幅44px・高さ36pxに揃える。各領域の中心を図柄の輪郭中心に合わせる。図柄の位置を維持するため領域の移動と逆向きにSVGを補正し、領域同士の間隔は均等にしない。リセットの選択領域は矢印を含む輪郭中心、回転軸は円弧の中心を使う。押すとモデルを即時に初期姿勢へ戻し、アイコンは円弧の中心を軸に650msで反時計回りに1回転してから非表示にする。回転中は連打を受け付けず、動きを減らす設定では即時に非表示にする。
...
- プレイ画面の左下に幅44px・高さ36pxの表示切替ボタンを絶対配置する。BUTTON CHECKは従来の右寄せフロー配置を維持し、切替アイコンは行の高さや上下余白に影響させない。正面の四角／等角投影の立方体を表示し、押すと反対の表示へ切り替える。設定画面と同期し、3Dタイプとゲーム状態は保持する。アイコンの面は64単位中、2Dの一辺32単位、3Dの一辺24.12単位を基準に、各状態の投影横幅を表示上で1px縮め、立体化に合わせて縮小する。SVG座標で中心(18, 46)を固定し、縮小・立体化ともに中心位置を維持する。幅44px・高さ36pxの操作領域は輪郭の中心に合わせ、絵の配置は維持する。SVGの輪郭はクリップしない。420msで形状を補間し、面内の2D／3D文字も面に追従して傾きながら切り替わる。文字は面の中心y=18にalphabetic基準線を指定し、dyは2Dが0.31em、3Dが0.35emとして直接補正し、親グループのdominant-baseline継承に依存させない。途中反転は現在の形から再開し、reduced-motionでは即時切替。
...
- スマホの3D表示領域は箱型（大ランプ）が左右10%余白（幅80%）、丸型が左右20%余白（幅60%）で中央に配置し、canvasは領域全体を使用する。モデルは余白8pxを残して最大表示する。左下のタイプ切替（3D時のみ）・2D／3D切替の位置は固定する。3Dモデルが初期姿勢から回転している間だけ、タイプ切替の上に姿勢リセットを表示し、Tab順も上から揃える。リセット後や手動で初期姿勢へ戻したときは非表示にする。判定はモデルごとの保存姿勢に従い、一周分の回転は初期姿勢と同じとみなす。テーマプレビューは初期姿勢のためリセットを表示しない。表示切替の位置を維持し、調整済みのスマホ41px間隔・PC53px間隔で並べる。タイプ切替は640msの面回転で、開始姿勢を描画後に回転し、完了後は3D変換を外してSVGを直接描画する。動きを減らす設定では即時切替。タイプ切替アイコンの丸型は仰角55°で上面と台座を投影し、上下端から中央位置を求めて余白を等しくする。箱型は10%拡大・線幅10%減、ランプ横径は元の素材より10%広げる。台座側面は底面を固定して表示上で0.5px厚くし、viewBoxは維持する。PCではタイプアイコン40px、表示切替SVG56px、スマホでは各32px・44pxとし、操作領域は幅44px・高さ36pxとする。
- 回転ドラッグは丸型の固定台座・箱型の箱本体に触れた場合だけ開始する。モデル全体へのレイキャストで最前面を判定し、背景・ランプ・キャップからは開始しない。開始後はポインターを捕捉し、台座の外へ移動しても回転を継続する。キャップの操作領域は最低44pxで、回転後の遮蔽判定を行う。正常なタッチ終了のpointerup→pointerleave→clickは受理し、ドラッグ・pointercancelは拒否する。DISABLED中も台座からの回転は可能だが、キャップ操作は回転へ読み替えない。設定・テーマ・横画面警告中はinertと入力ガードで操作を止める。
...
- **Type**: boolean
- **When true**: ANSWERING状態への遷移と同時にYouTube動画プレイヤーを隠し、「解答中・・・」表示に置換する（演出待ちなし。iframe自体は破棄せずプレイヤー状態を保持する）。タッチ端末では既存どおり動画領域を畳む。
...
- **タッチターゲット**: 原則44px以上。解答入力欄・送信ボタンは縦幅確保のため最低36pxとする。左下のアイコン群は幅44pxを保ち、表示・タイプ切替・姿勢リセットとも高さ36pxとする
