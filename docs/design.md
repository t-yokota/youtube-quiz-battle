...
- PCの3D canvasはgame-ui全体に重ねて配置する。従来のモデル領域は中心位置の基準として残し、その中心からgame-uiの端まで収まる範囲でサイズを再計算するため、描画領域が広がった分だけ拡大を許可する。解答欄展開時は中心の移動に連動して利用可能範囲を縮める。解答欄・タイプ切替・表示切替・BUTTON CHECKはcanvasより前面に置く。
