...
- PCの箱型モデルは、初期角度での投影幅を、その画面での初期game-ui幅の25%以下に制限する。境界ドラッグでは上限を変更せず、幅を狭めたときの「拡大→一定」を保つ。ただし領域に収まらなくなる場合は縮小する。ウィンドウ寸法変更時は13:12の初期幅から上限を再計算する。固定pxの高さ上限は設けず、縦横比と表示中心は維持する。
- PCの丸型モデルは、初期角度での投影幅を、その画面での初期game-ui幅の30%以下に制限する。箱型と同様、境界ドラッグでは上限を変更せず、ウィンドウ寸法変更時に再計算する。canvas自体はgame-ui全体を使い、上限未満では領域への追従を維持する。上限適用時も表示中心を動かさない。
...
- 主ポインターが`fine`で幅960〜1199pxの場合、左サイドのみ非表示にした中間レイアウトを使う。中央の左端を画面左に固定し、右サイドを240px以上確保する。動画最小幅は640px、右境界のドラッグ量をそのまま動画幅に反映し、左境界は操作・Tab対象から外す。設定サイドメニューとPCテーマプレビューも維持する。幅960px未満または主ポインターが`fine`でない場合はスマホの縦積み配置に切り替える。
- 動画の自動初期幅には、上記の高さ基準に加えて画面幅の60%上限を適用する。最小640pxとサイドの必要幅は維持し、手動ドラッグでは60%上限を適用しない。3Dモデル上限の基準となる初期幅とテーマプレビューにも同じ計算を使う。
