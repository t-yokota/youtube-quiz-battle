...
- [x] 29. デバッグモード（クイズ設定の実行時上書き）※2026-07-05 追加・完了
  - 実行 spec: [improvement/specs/task29-debug-mode.md](improvement/specs/task29-debug-mode.md)
  - settings.debug: true のデータのみ設定モーダルにデバッグセクション（ヘッダー左トグルで表示切替）
  - debugStore（セッション限り）+ gameStore.effectiveSettings で上書きを一元解決
- [x] 25. Firebase Analytics 統合（旧: 25）※実装完了（2026-07-05）。Firebase Console でのプロジェクト作成 + src/constants/firebase.ts への config 貼り付け、DebugView 確認、GA4 UI 登録（spec 25-6）が残ユーザー作業※実行 spec: [improvement/specs/task25-firebase-analytics.md](improvement/specs/task25-firebase-analytics.md)（2026-07-05 作成）
...
- [x] 30. quizId パラメータ対応（?quiz= でコンテンツ指定）※2026-07-05 追加・完了
  - 実行 spec: [improvement/specs/task30-quiz-id-param.md](improvement/specs/task30-quiz-id-param.md)
...
- [~] 26. E2E テストスイート（旧: 26 + 23-2 統合）※**不採用（2026-07-06 ユーザー裁定）**
  - アプリ規模に対して維持コストが見合わないため Playwright 導入を見送り。
    ...
    [improvement/specs/task26-e2e-suite.md](improvement/specs/task26-e2e-suite.md) に保存済み。将来必要になれば復活可能）
