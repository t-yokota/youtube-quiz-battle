...
**完了の定義**:
- [ ] `AnswerValidator` インターフェース実装
- [ ] 完全一致判定（正規化なし）が動作する
- [ ] 複数正解パターン（配列）に対応
- [ ] trim + NFKC 正規化を内部で用意（既定OFF）
- [ ] 正誤判定結果がスコアに反映される
...
**完了の定義**:
- [ ] スペースキーで早押しボタンが押下できる
- [ ] `input`/`textarea`/`contentEditable` 入力中は誤操作防止で無効化
- [ ] `preventDefault()` でスペースキーによるスクロールを抑止
- [ ] ボタン状態遷移（STANDBY → PUSHED → RELEASED → DISABLED）が完全動作
- [ ] PC環境でのキーボードとクリック操作が統合されている
