...
**完了の定義**:
- [x] ゲーム状態に応じた全コンポーネントの表示切り替えが動作する
- [x] 解答カウントダウンタイマーが ANSWERING で開始・External Pause で停止/再開する
- [x] 解答送信フローが完成（入力 → 判定 → 結果表示 → 動画再開）
- [x] GameInfo（問題番号・スコア）がリアルタイム更新される
- [x] ResultArea（FinalScore・ResultTable・ResultActions）が FINISHED 状態で表示される
- [x] 「もう一度プレイ」ボタンで最初から再プレイできる
...
**完了の定義**:
- [x] GameManager / gameStore レベルのゲームフロー統合テスト
- [x] 解答送信フローのテスト（GameManager 経由）
- [x] タイマー機能のテスト
