...
> **A〜E実装完了（2026-09-11）**: ユーザー承認に基づき、修正範囲ごとにコミットして実施。Fの未使用資産整理・80%ゲート・ローカルPWA確認も実施済み。実機・PWA更新等は未確認として現役タスクに記録。設計書分割・クイズ作成スキルは対象外、ルール同期・設計同期スキルは保留。GA4の既存アプリ実装範囲は従来の動作確認済み判断を維持する。
...
以下は**初回レビュー時の指摘**。A〜C対象については、その後の実装で回帰テストを追加して修正済み。参照行はレビュー時のHEAD時点であり、現在の行番号とは一致しない。
...
- 各PR: 非破壊Lint、型、build、test、coverageを実施。デプロイ権限はデプロイjobへ限定する。Aでworkflowを変更済み。push・デプロイ・リモートでのworkflow実行は今回行っていない。

## 文書・運用の整理と判断事項

- 旧R計画は実施済みの履歴として保持し、今回の計画を新たな改善サイクルとして扱う。R-6の分割を再実施しない。
- 旧`docs/tasks.md`のPhase 4・330件・Firebase設定待ち等の記述は、旧計画整理で履歴へ移動済み。GA4のアプリ実装範囲はユーザー認識に基づき動作確認済みとして終了した。A〜Cの実行記録は現役`docs/tasks.md`へ登録済み。
- `docs/design.md`は分割しない（ユーザー決定）。参照先は現配置へ更新済み。今後のコード変更に伴う内容の更新は同ファイルに反映する。ルール同期・設計同期スキルの作成は保留。
- READMEのクイズ指定URL、作成例、開発・検証コマンド、設計・デプロイへのリンクは更新済み。`src/assets/base.css`・`logo.svg`はFで参照がないことを確認して削除済み。
...
A〜Eの実装とFのローカル作業を実施した。次は[現役タスク](tasks.md)に残した実機・PWA更新等の確認。

## A〜Cの実施記録（2026-09-11）

| 修正範囲 | 対応 |
|---|---|
| 品質基盤（F-14） | 非破壊Lint、全ソースcoverageと初回値の下限、PR/develop品質workflow、デプロイ権限分離、状態と時刻が連動するPlayerフェイク |
| 入力（F-01・02） | ゲート・overlay時の操作遮断、IME確定Enterの除外 |
| ゲーム進行（F-03・04・10・11） | 内部シークの到達待機と期限、150ms監視での停滞検出と復帰、開始操作後の0秒問題、問題別の試行情報 |
| 境界とエラー（F-05・06・08・09） | Playerのready期限と実行時エラー、保存不能時の継続、unknown JSONの構造・値検証 |
| 終了処理（F-07・13） | 全ボタンタイマーの取消、遅延通知の失効、Playerの生成途中abort、音声dispose・resume取消、クイズ取得と再試行待機のabort |
| 文書 | READMEの品質コマンド、designの実装契約、現役tasksの進捗を同期。設計同期スキルは作成しない |

コミットは段階単位ではなく修正範囲ごとに作成した。初期化エラーが遅れて発生した場合もゲームループ・動画・効果音を止める統合テストを追加した。

最終検証値は[現役タスク](tasks.md)に記録する。実YouTube・iOS/Android・PWA更新・本番GA4の再検証は未実施。F-12のフォーカス閉じ込め・復元は後続のEで実施済み。全指標80%はFの完了条件であり、A〜C完了とは区別する。

## Dの実施記録（2026-09-11）

- `useQuizAnalytics`へ設定監視・イベント生成・セッションIDと送信済み件数を抽出。共通イベント項目とシーク設定の解決を集約した。
- `useQuizSession`へデータ取得・Player接続・GameManager/ループ・音声設定・エラー停止・終了処理を抽出した。Appは画面構成と操作接続を担当する。
- Appの統合テストで開始設定、複数試行、スキップ/無解答、timeout、完走集計、リプレイの新規セッションID、送信回数とunmount後の送信停止を確認した。
- セッションのテストで初期化中の終了と遅延通知、音声設定、同期priming、早押し/解答、画面向き復帰を確認した。iOSの同期focus・warmup・unlockは維持し、実機での再検証はFに残す。

実行結果は[現役タスク](tasks.md)へ記録する。Dは単体作業で実施し、テスト・Analytics抽出・セッション抽出・文書同期を別コミットにした。

## Eの実施記録（2026-09-11）

- `useModalLayer`を共通基盤として、名前・role・最前面のフォーカス循環・背景遮断・終了時の復元を実装した。既存DOMとレイアウトを維持するためラッパー追加ではなくcomposableで共通化した。
- 設定・テーマ選択にはEscapeを追加し、読み込み・横画面警告・Appのエラーは閉じられない条件を維持した。エラーを最前面へ揃え、警告表示中の背面操作を検証した。
- 設定からテーマへ移る経路、上位警告が重なる経路、動的な要素削除、unmount時の解除を回帰テストで保護した。テーマ選択の取消時は遅れた適用を止める。
- `SettingsGeneral`と`SettingsDebug`へ入力処理を分割し、操作・項目順序・CSS縮尺を検証した。
- 全体検証値は[現役タスク](tasks.md)へ記録する。実機・PWA・スクリーンリーダーでの確認は未実施で、Fに残す。


## Fの実施記録（2026-09-11）

- 未参照のテンプレート資産`base.css`・`logo.svg`を削除し、coverage下限を全指標80%へ引き上げた。
- 全559テスト、型チェック、build、Lintと80%ゲートを通過した。
- 本番ビルドをローカルChromiumで開き、Service Worker登録・Manifest/アイコン配信・オフライン再表示を確認した。外部通信遮断時のYouTube読み込みエラー表示も確認した。
- 実機・読み上げ・新旧ビルド間のPWA更新は未確認。[確認手順](local-device-testing.md#リファクタリング後の実機チェック)と[現役タスク](tasks.md)へ残した。Fのローカル作業完了と実機での動作保証は区別する。
