...
作成日: 2026-09-14。対象: develop。状態: 実装・ローカル検証完了。実機確認・公開は未実施。

## 結論

提供キットの形状・アニメーション計算を再利用し、現在のQuizButtonに3D表示を追加する。受理判定・動画停止・効果音・解答期限は既存のApp／GameManagerが担当する。現行2Dを残し、設定から丸型／箱型（大ランプ）を選べる。プレイ画面のアイコンからも2D／3Dを切り替えられる。
...
| 発光 | RELEASED中は500ms周期。丸型はキャップ、箱型（大ランプ）は固定ランプのみ |
...
| 高 | 箱型（大ランプ）の押下対象は小キャップのみで、スマホ上の実寸が小さくなる可能性 | 320px幅で画面上の当たり判定を測定。必要なら遮蔽を考慮した許容領域か明示的なDOM操作ボタンを用意。ランプや回転操作が早押しになる拡張は避ける |
...
1. 初期値は従来2D。設定を「表示方式: 2D／3D」と、条件付きの「ボタンタイプ: 丸型／箱型（大ランプ）」を同じ字下げで並べる。ボタンタイプは3D選択時だけ表示し、2Dへ戻しても最後に選んだ3Dタイプを保存する。3Dモデルの色・形・照明はADDONを基準とし、DISABLED時の減光を強める。周辺UIだけ既存テーマに従う。
2. 表示設定は内部ではrenderModeとmodelId、固定appearancePresetIdを分離する。保存データにバージョンを持たせ、既存の音声・シーク等の設定を保持して移行する。未知ID・破損値は2Dへ戻す。
3. 表示方式・ボタンタイプはプレイ中も変更可能にする（開始後に選択できないという報告を受け、初期のREADY／FINISHED制限を撤廃）。既存の設定ダイアログの動画停止・解答期限継続ポリシーを維持する。
4. 押下は既存press経路に同期接続し、描画完了・nextTick・importの完了を待たせない。PUSHED／RELEASEDを同期watchで受け、ゲームが受理する前には演出しない。
5. 既存2Dの再生アイコンとBUTTON CHECK切替を維持する。3Dには再生アイコンも重ねない。ユーザー指定により3DのPUSH／ON!／WAIT文字とモデル上の文字は追加せず、aria-labelで操作名を提供する。READYのチェックOFF時に押下演出を追加するためのゲーム変更は行わない。
...
3Dの初期有効化、押下タイミングのpointerdown化、調整用スライダー、JSON入出力、GLB対応は初回対象に含めない。
...
| ボタン状態 | 3Dの見た目 | 操作 |
| --- | --- | --- |
| STANDBY | 通常位置・通常の素材色・消灯 | 文字なし。READYかつチェックOFFでもモデル上の表示は重ねない。入力可否は既存のisButtonEnabledに従う |
| PUSHED | 受理済み押下の単発アニメーション | 追加押下を受理しない |
| RELEASED | 丸型はキャップ、箱型（大ランプ）はランプが点滅 | 文字なし。入力不可でも点灯は維持 |
| DISABLED | 可動部を通常位置へ戻し、キャップを減光、点滅停止。箱型（大ランプ）はランプも減光・消灯 | 文字なし。早押し操作を無効化 |
...
`isButtonEnabled=false`を一律DISABLEDへ変換してはいけない。受理直後や解答中のRELEASEDまでWAITにすると、解答権取得の点灯が消えるため、表示状態と入力可否を別々に渡す。既存2DのDISABLEDは沈み込み表示だが、3Dはキットの通常位置＋減光を採用する。

## 実装順序と完了条件

### 1. 描画基盤とモデルの移植

- `src/components/game/button3d/`へ型契約、設定、motion、glow、registry、2モデル、viewを移植する。
...
- `Button3DView.vue`をVueアダプターとし、QuizButtonを2D／3Dの表示切替の窓口にする。既存2D表示は必要な範囲で子コンポーネントへ抽出する。
...
- Three.js / @types/threeは0.186.0。モデル・型・コンポーネントの名称をbuttonへ統一し、保存済みの旧buzzer設定は読み込み時に引き継ぐ。
- タッチ終了時のpointerup→pointerleave→clickを許可し、ドラッグ・pointercancelは拒否する。モデル上に文字・再生アイコンは重ねない。
- 形状頂点と押下範囲の投影輪郭を中央に合わせ、余白8pxで最大表示する。canvas幅は箱型（大ランプ）が親幅の80%、丸型が60%。resetは切替アイコンの上に配置する。
- DISABLEDでは両モデルのキャップ色を0.25倍、箱型のランプ色を0.12倍にする。ランプのroughnessを0.6へ上げて反射を抑え、通常時は0.23へ戻す。
- プレイ画面の表示切替は文字付きのモーフィングアイコン。形状・配置・タップ領域の現行値は[設計書](design.md#3d早押しボタン2026-09-14)に集約する。BUTTON CHECKのフロー配置と高さは維持する。
- 設定はプレイ中も変更できる。タイプ名は丸型／箱型（大ランプ）。選択中の文字幅に追従し、枠はアクセント色、説明は常にセクション最下部。行間は共通gapを使う。
- 全622テスト成功。coverage: Statements 91.64% / Branches 84.96% / Functions 92.17% / Lines 94.05%。型チェック・Lint・本番ビルド成功。オンライン依存監査0件。
- ローカルChromiumでタッチ、モデル切替、設定、アイコン、幅320／390／768pxを確認（YouTube模擬）。初期導入時にはWAITとWebGL障害後の2D復帰も確認。
- 3Dチャンクは約534KB（gzip約136KB）、PWAのprecacheは約2MB。既存の500KBチャンク警告は残る。
- 実YouTube・iOS／Android・読み上げ・実端末GPU負荷・HTTPSのPWA更新は公開後の確認対象。ローカル検証で代替しない。
