...
describe('PWA configuration', () => {
  ...
  it('iOS向けアイコンとPWA派生アイコンを用意する', () => {
    const assets = [
      ...
      'public/assets/icons/favicon-48x48.png',
      'public/assets/icons/icon-192x192.png',
      'public/assets/icons/icon-512x512.png',
      'public/assets/icons/icon-maskable-512x512.png',
    ...
    for (const obsoleteAsset of [
      ...
      'public/assets/icons/icon-16x16.png',
      ...
      'public/assets/icons/pwa-maskable-512x512.png',
    ]) {
      expect(existsSync(resolve(root, obsoleteAsset)), obsoleteAsset).toBe(false)
    }
    expect(indexHtml).toContain('<link rel="apple-touch-icon" href="/assets/icons/apple-touch-icon.png" />')
    expect(viteConfig).toContain('src: `${BASE_PATH}assets/icons/icon.svg`')
    expect(viteConfig).toMatch(
      /src: `\$\{BASE_PATH\}assets\/icons\/icon\.svg`,\s+sizes: 'any',\s+type: 'image\/svg\+xml',\s+purpose: 'any maskable'/,
    )
    expect(viteConfig).toMatch(
      /src: `\$\{BASE_PATH\}assets\/icons\/icon-192x192\.png`,\s+sizes: '192x192',\s+type: 'image\/png'/,
    )
    expect(viteConfig).toMatch(
      /src: `\$\{BASE_PATH\}assets\/icons\/icon-512x512\.png`,\s+sizes: '512x512',\s+type: 'image\/png'/,
    )
    expect(viteConfig).toMatch(
      /src: `\$\{BASE_PATH\}assets\/icons\/icon-maskable-512x512\.png`,\s+sizes: '512x512',\s+type: 'image\/png',\s+purpose: 'any maskable'/,
    )
  ...
  it('favicon用PNGとManifest用PNGの実寸をファイル名に揃える', () => {
    const dimensions = [
      ['public/assets/icons/favicon-16x16.png', 16],
      ['public/assets/icons/favicon-32x32.png', 32],
      ['public/assets/icons/favicon-48x48.png', 48],
      ['public/assets/icons/icon-192x192.png', 192],
      ['public/assets/icons/icon-512x512.png', 512],
      ['public/assets/icons/icon-maskable-512x512.png', 512],
    ] as const
    ...
    for (const size of [16, 32, 48]) {
      ...
      expect(icon?.getAttribute('href')).toBe(`/assets/icons/favicon-${size}x${size}.png`)
    }
