...
describe('SettingsModal', () => {
  ...
  it('設定画面専用のレイアウト・文字・操作部unitを定義する', () => {
    ...
    expect(compactOverlay).toContain(
      '--settings-layout-unit:clamp(var(--settings-layout-min),calc(min(100dvh/700,100vw/315)*16),var(--settings-layout-max))',
    )
    expect(compactOverlay).toContain(
      '--settings-font-unit:clamp(var(--settings-font-min),calc(min(100dvh/700,100vw/315)*16),var(--settings-font-max))',
    )
