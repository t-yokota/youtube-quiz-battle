...
it('表示方式→3Dタイプの階層を表示し、プレイ中は変更を受け付けない', async () => {
  ...
  app.mount(host)
  cleanup = () => app.unmount()
  const mode = host.querySelector<HTMLSelectElement>('#buzzer-mode')!
  expect(host.querySelector('#buzzer-model')).toBeNull()
  mode.value = '3d'
  mode.dispatchEvent(new Event('change'))
  await nextTick()
  const model = host.querySelector<HTMLSelectElement>('#buzzer-model')!
  model.value = 'waseda-style-v1'
  model.dispatchEvent(new Event('change'))
  expect(settings.buzzer.modelId).toBe('waseda-style-v1')
  mode.value = '2d'
  mode.dispatchEvent(new Event('change'))
  await nextTick()
  expect(host.querySelector('#buzzer-model')).toBeNull()
  ...
  mode.dispatchEvent(new Event('change'))
  await nextTick()
  expect(host.querySelector<HTMLSelectElement>('#buzzer-model')!.value).toBe('waseda-style-v1')
  game.transitionToState(GameState.QUESTIONING)
  await nextTick()
  expect(mode.disabled).toBe(true)
  mode.value = '2d'
  mode.dispatchEvent(new Event('change'))
  const locked = host.querySelector<HTMLSelectElement>('#buzzer-model')!
