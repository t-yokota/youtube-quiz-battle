...
describe('実画面のレスポンシブスケール', () => {
  ...
  it('テーマプレビューのchip・早押しボタン・トグル寸法を実画面のunitへ揃える', () => {
    ...
    expect(quizButton).toContain('height: calc(9.375 * var(--button-unit))')
  ...
  it('テーマプレビューの主要レイアウト寸法を実画面と同じlayout unitへ揃える', () => {
    ...
    expect(selectorBlock(themePreview, '.p-panel')).toContain('min-height: 6.875rem')
    expect(selectorBlock(themePreview, '.p-panel')).toContain('padding: 0.75rem 0.875rem')
    expect(selectorBlock(themePreview, '.p-input')).toContain('height: max(36px, 2.5rem)')
    expect(selectorBlock(themePreview, '.p-submit')).toContain('height: max(36px, 2.5rem)')
  ...
  it('文字を分離してもパネル・リング・ボタンの寸法はlayout remで縮尺する', () => {
    ...
    expect(selectorBlock(quizButton, '.quiz-button')).toContain('width: calc(9.375 * var(--button-unit))')
