...
[公開アプリ](https://youtube-quiz-battle.pages.dev/)を開き、読み込み後に「タップしてはじめる」を押す。ボタンを押して動画の再生を開始し、出題される問題に早押しで解答する。設定から効果音・シーク許可・ボタンチェック演出・UIテーマを変更できる。ボタンの表示方式（2D／3D）は設定画面やプレイ画面左下のアイコンで切り替えられる。3Dでは丸型／箱型（大ランプ）のタイプを選択し、台座や余白をドラッグして回転できる。3Dが利用できない場合は2Dで表示する。
